...
void CollisionManager::GetWorldMouse(void) // 마우스 월드포지션
{
	POINT pt = {};
	GetCursorPos(&pt);
...
void CollisionManager::CheckCollision(ObjectTag _src, ObjectTag _dst)
{
	for (auto& srcobj : colliderdatas[_src])
	{
		for (auto& dstobj : colliderdatas[_dst])
		{
			...
			{
				srcobj->CollisionEvent(dstdata->tag, dstobj); // 해당 오브젝트 콜리전 이벤트 실행
				dstobj->CollisionEvent(srcdata->tag, srcobj); // 해당 오브젝트 콜리전 이벤트 실행
			}
		}
	}
}
	...
	out D3DXVECTOR3& _pos2, in ObjectTag _dsttag)
{
	...
	for (auto& dst : colliderdatas[_dsttag])
	{
		...
		if (orthogonallengthsqare <= radiusquare)
		{
			...
			_targetposition = dynamic_cast<Transform*>(dst->GetComponent(L"Transform"))->localPosition;
			dstObject = dst;
...
GameObject* CollisionManager::GetClosestObject(in ObjectTag _dsttag, in const D3DXVECTOR3& _srcpos, in const D3DXVECTOR3& _srcdir, in const float _angle)
{
	...
	for (auto& dst : colliderdatas[_dsttag])
	{
		...
		D3DXVECTOR3 dstpos = dynamic_cast<Transform*>(transform)->localPosition;
